...
function useSidebarRender<RenderedElement extends Element>({
  asChild,
  defaultTagName,
  ref,
  props,
  ...
  ref: React.Ref<RenderedElement>;
  props: Record<string, unknown> & { children?: React.ReactNode };
}) {
  const { children, ...slotProps } = props;

  return useRender<Record<string, unknown>, RenderedElement>({
    defaultTagName,
    render: asChild
      ? (React.Children.only(children) as React.ReactElement)
      : undefined,
    ref,
    props: asChild ? slotProps : props,
  });
}
      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider>
          <div
            ...
            className={cn(
              'group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar flex items-center justify-center', // Removed w-full
              className,
            )}
            ref={ref}
            {...props}
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            'relative bg-transparent transition-[width] duration-300',
            'group-data-[collapsible=offcanvas]:w-0',
            'group-data-[side=right]:rotate-180',
            variant === 'floating' || variant === 'inset'
              ? 'w-[calc(var(--sidebar-width)+1rem)] group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4)))]'
        ...
        <div
          className={cn(
            ...
            side === 'left'
              ? variant === 'floating'
                ? 'left-0 group-data-[collapsible=offcanvas]:-left-[(var(--sidebar-width)+1rem)]'
                : 'left-0 group-data-[collapsible=offcanvas]:-left-(--sidebar-width)'
              : variant === 'floating'
                ? 'right-0 group-data-[collapsible=offcanvas]:-right-[(var(--sidebar-width)+1rem)]'
                : 'right-0 group-data-[collapsible=offcanvas]:-right-(--sidebar-width)',
            // Adjust the padding for floating and inset variants.
            variant === 'floating' || variant === 'inset'
              ? 'p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4))+2px)]'
              : 'border-sidebar-border group-data-[collapsible=icon]:w-(--sidebar-width-icon) group-data-[collapsible=offcanvas]:border-0! group-data-[side=left]:border-r group-data-[side=right]:border-l',
            className,
          )}
          {...props}
        >
...
>(({ className, asChild = false, ...props }, ref) => {
  return useSidebarRender<HTMLDivElement>({
    asChild,
    defaultTagName: 'div',
    ref,
    props: {
...
>(({ className, asChild = false, ...props }, ref) => {
  return useSidebarRender<HTMLButtonElement>({
    asChild,
...
>(
  (
    ...
    const button = useSidebarRender<HTMLButtonElement>({
      asChild,
      defaultTagName: 'button',
      ref,
      props: {
...
>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      'bg-sidebar-accent/50 flex min-w-0 flex-col gap-1 rounded-b-[18px] p-2.5',
      'group-data-[collapsible=icon]:hidden',
      className,
    )}
    {...props}
...
>(({ asChild = false, size = 'md', isActive, className, ...props }, ref) => {
  return useSidebarRender<HTMLAnchorElement>({
    asChild,
    defaultTagName: 'a',
    ref,
    props: {
